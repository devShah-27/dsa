...
// The top view contains the first visible node at each horizontal distance, ordered from the leftmost distance to the rightmost distance.
...
- Store only the first node encountered at each horizontal distance, because it is the topmost visible node.
...
vector<int> topView(TreeNode *root)
{
    ...
    // Stores horizontal distance -> first visible node value.
    map<int, int> nodeMap;
    ...
    while (!q.empty())
    {
        ...
        // Store only the first node encountered at this distance.
        if (nodeMap.find(vertical) == nodeMap.end())
    ...
    // Map automatically stores horizontal distances in sorted order.
...
int main()
{
    ...
    vector<int> topViewList = topView(root);

    cout << "Top View Traversal: " << endl;

    for (auto node : topViewList)
    {
        cout << node << " ";
    }
